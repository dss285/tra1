package tra1.vk7.xtehtava;// TRAI_20_X7_testi.java SJ

import java.util.*;

public class TRAI_20_X7_testi {


	static TRAI_20_X7 testattava = new TRAI_20_X7_tommpuur(); /* <-- Oma tunnus tÃ¤hÃ¤n */
	...
	public static void main(String[] args) {
		...
		ok &= testaaX7(0, 3, 2, false, print);
		ok &= testaaX7(1, 3, 2, false, print);
		ok &= testaaX7(2, 5, 2, false, print);
		ok &= testaaX7(3, 6, 3, false, print);
		ok &= testaaX7(4, 0, 2, false, print);
		ok &= testaaX7(N, M, K, false, print);
		ok &= testaaX7(N, M, K, true, print);
		...
		int virheet = 0;
		for (k = 0; k < nTest; k++) {
			if (!testaaX7(rnd.nextInt(k+1), rnd.nextInt(k+1)*2, 4, rnd.nextBoolean(), 0))
				virheet++;
			if (virheet >= 30) {
				k++;
				break;
	...
	static boolean testaaX7(int joukkoja, int alkioita, int kerroin, boolean uniikkiAlkio, int print) {

		boolean ok = true;

		if (print > 0) System.out.println("\n===========\ntestaaX7 " + joukkoja + " " + alkioita + "\n");
	...
	/**
	 * Generoi syÃ¶tteen ja verrokkituloksen tehtÃ¤vÃ¤Ã¤n X7.
